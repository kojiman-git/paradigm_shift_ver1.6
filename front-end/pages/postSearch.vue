...
import axios from 'axios';
  export default {
    ...
    created(){
      axios
      .get(`https://test-ecs-back-end.work/search/narrow_down_by_category?post%5Bm_category_id%5D=${this.filter}`, { withCredentials: true })
      .then(response => {
    ...
    filterEvent(ID){
        axios
          .get(`https://test-ecs-back-end.work/search/narrow_down_by_category?post%5Bm_category_id%5D=${ID}`,{ withCredentials: true })
          .then(response => {
            this.filter = ID
            this.$data.postList =  response.data 
            console.log(this.$data.postList);
